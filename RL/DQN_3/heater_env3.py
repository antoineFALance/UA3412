...
class heaterEnvRC(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    ...
    def __init__(self):
        ...
        self.data['consigne'] = np.where((self.data['hour'] >= 20), 0, self.data['consigne'])
        self.data['consigne'] = np.where((self.data['hour'] <= 5) & (self.data['hour'] <= 23), 0, self.data['consigne'])
    ...
    def step(self,action):
        ...
        terminated=bool(self.t==200)
        if self.consigneList[self.t]==18:
            terminated=bool(Tint1 < 0.95 * self.consigneList[self.t])
        if not terminated:
            # reward=-math.tanh(abs((Tint1-self.data['consigne'].to_list()[self.t]))-2)
            reward=1*math.exp(-0.03*action)
            self.t += 1
